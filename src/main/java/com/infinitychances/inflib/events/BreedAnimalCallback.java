...
public interface BreedAnimalCallback {
    ...
    Event<BreedAnimalCallback> EVENT = EventFactory.createArrayBacked(BreedAnimalCallback.class,
            (listeners) -> (player, animal, other, baby, type) -> {
                for (BreedAnimalCallback listener : listeners) {
                    ActionResult result = listener.interact(player, animal, other, baby, type);
     ...
     * @param type   the type of animal entity? being bred
     *
     ...
     */
    ActionResult interact(PlayerEntity player, AnimalEntity animal, AnimalEntity other, PassiveEntity baby, EntityType<?> type);
}
